  ...
  const connectFacebook = useCallback(async () => {
    ...
    try {
      ...
      const response = await apiClient.request<{ url: string }>(
        `/v1/oauth/facebook/login-url?origin=${encodeURIComponent(origin)}`,
      );

      if (!response.ok || !response.data?.url) {
        toast.error(response.errors?.[0]?.detail ?? 'Unable to start Facebook connection.');
      ...
      const handleMessage = (event: MessageEvent) => {
        ...
        if (payload.status === 'success') {
          ...
          toast.error(payload.message ?? 'Facebook connection was cancelled.');
        }
      ...
      console.error('[useSocialAccounts] connectFacebook error', error);
      toast.error('Unable to connect Facebook account.');
      setIsConnecting(false);
    }
  }, [isConnecting, load]);
